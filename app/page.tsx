  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (tg) {
        ...
        // fallback for development (when not running in Telegram WebApp)
        setIsTelegram(false);
        setTheme('dark');
        setUser({
  ...
  return (
    <main className={`min-h-screen ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-300">
        <button onClick={() => (window as TelegramWindow).Telegram?.WebApp?.close()} className="text-sm">
          ❌ Close
        </button>
        <h1 className="text-lg font-semibold">TeamXcodex</h1>
        <button onClick={() => alert('Menu clicked')} className="text-xl">
          ⋮
        </button>
      </div>
